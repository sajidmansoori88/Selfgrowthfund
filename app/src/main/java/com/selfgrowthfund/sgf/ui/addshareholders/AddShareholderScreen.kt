...
fun AddShareholderScreen(
    ...
    LaunchedEffect(saveSuccess) {
        saveSuccess?.let { success ->
            if (success) {
                Toast.makeText(context, "Shareholder added successfully!", Toast.LENGTH_SHORT)
                    .show()
    ...
    GradientBackground {
        Column(
            modifier = modifier
                ...
                .padding(16.dp) // Moved padding to the modifier chain
        ) {
            // Display next ID
            ...
            // ─────────────── Date of Joining ───────────────
            val joiningText =
                joiningDate?.format(DateUtils.formatterPaymentDate) ?: "Select Joining Date"
            Box(modifier = Modifier.fillMaxWidth()) {
                OutlinedTextField(
                    value = joiningText,
                    onValueChange = {},
